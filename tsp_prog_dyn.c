...
double tsp_prog_dyn(point *V, int n, int *Q) {
  ...
  */
  ...
  for(int S=1; S<C; S++){
    for(int t=0; t<L; t++){
      if(DeleteSet(S, t) != S){ // test si t est dans S, si oui on est sur case blanche
        int T = DeleteSet(S,t); // T: ensemble S privé de t
        if(T == 0){ // si S contient seulement un élément (t)
          D[t][S].length = dist(V[n-1], V[t]);
          D[t][S].pred = n-1;
        } else {  // si S a plus d'un element
          double min =  DBL_MAX;
          int pred_min = n-1;
          for(int x=0; x<L; x++){
            if(DeleteSet(T, x) != T){
  ...
  if (running) {
    ...
    int minT = n;
    for(int t=0; t<L; t++){
      double val = D[t][C-1].length + dist(V[t], V[n-1]);
      if(val < w){
        w=val;
        minT=t;
      }
